...
from torch.autograd import Variable
import eval_slots
from model import  model_experiement, model_infer_slot_sam2
...
CLIENT_ID = 12
DEFAULT_CONFIG = "videosaur_m/configs/videosaur/ytvis3_MLP_en_15.yml"
...
Model_infer = model_infer_slot_sam2._Model_infer(parser.parse_args(),parser_s.parse_args(),GPU_mode,num_gpus,slot_ini="random",Foundation_M="ensemble",foundation_list  = ["DINO","SAM","MAE","CLIP"],fusion_method='moe_layer',gpu_selection=Gpu_selection,TPC=True)
...
while (1):
    ...
    if (read_id % Fed_iter) == 0 and not Evaluation_slots:
        # Save client model
        client.save_client_model(Model_infer.model,participate=False)
        client.save_client_model_s(Model_infer.model_s,False)
