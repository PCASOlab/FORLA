...
Output_root = Output_root + "ensemble_SA/"
# GPU_mode= True
# Continue_flag = True
# Visdom_flag = False
# Display_flag = False
# loadmodel_index = '3.pth'
# Output_root = Output_root+ "Obj_centric_dinosau" + selected_data + str(Data_percentage) + "/"
# io.self_check_path_create(Output_root)
dataset_tag = "+".join(selected_data) if isinstance(selected_data, list) else selected_data
Output_root = Output_root+ "dino_onlysau786" + dataset_tag + str(Data_percentage) + "/"
io.self_check_path_create(Output_root)
...
DEFAULT_CONFIG = "videosaur_m/configs/videosaur/ytvis3_MLP64.yml"
...
Model_infer = model_infer_slot_att._Model_infer(parser.parse_args(),GPU_mode,num_gpus,slot_ini="random",Foundation_M="ensemble",foundation_list=["DINO"],fusion_method="concat",gpu_selection=Gpu_selection,pooling="max",TPC=True)
...
if Continue_flag == False:
    ...
    Model_infer.model.adapter.load_state_dict(torch.load(Output_root + 'adapter' + loadmodel_index,map_location='cuda:0' ))
...
while (1):
    ...
    if (read_id % 1000) == 0  :
        torch.save(Model_infer.model.adapter.state_dict(), Output_root + "adapter" + str(saver_id) + ".pth")
        torch.save(Model_infer.model.initializer.state_dict(), Output_root + "initializer" + str(saver_id) + ".pth")
        torch.save(Model_infer.model.encoder.state_dict(), Output_root + "encoder" + str(saver_id) + ".pth")
        torch.save(Model_infer.model.processor.state_dict(), Output_root + "processor" + str(saver_id) + ".pth")
        torch.save(Model_infer.model.decoder.state_dict(), Output_root + "decoder" + str(saver_id) + ".pth")
