...
def random_crop_and_flip(video, min_crop_ratio=0.3):
    """
    Random crop and horizontal flip for a video of shape [3, D, H, W].
    The crop size is randomly chosen but not smaller than min_crop_ratio * original size.
    
    Args:
        video (np.ndarray): Video tensor [3, D, H, W]
        min_crop_ratio (float): Minimum crop size ratio (e.g., 0.3 for 30%)
        
    Returns:
        np.ndarray: Transformed video [3, D, H, W]
    """
    C, D, H, W = video.shape
    
    # Random crop size between 30% and 100% of original
    crop_h = randint(int(H * min_crop_ratio), H)
    crop_w = randint(int(W * min_crop_ratio), W)

    # Random crop position
    top = randint(0, H - crop_h)
    left = randint(0, W - crop_w)

    cropped = video[:, :, top:top + crop_h, left:left + crop_w]

    # Resize back to original shape
    cropped_resized = resize(cropped, (C, D, H, W), preserve_range=True).astype(video.dtype)

    # Random horizontal flip
    if random() > 0.5:
        cropped_resized = cropped_resized[..., ::-1]  # Flip along width axis
