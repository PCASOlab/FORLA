...
# Resize transformer
    ...
    def forward(self, input, batch_files=None, features=None, epoch=None, read_id=None,
        ...
        # Prepare SAM input (denormalize to 0-255 range)
        sam_input = (self.input_resample * 60.0 + 124.0).clamp(0, 255).byte()
        
        # Initialize container for SAM masks [bz, 11, D, H, W]
        sam_masks = torch.zeros((bz, 11, D, self.input_size, self.input_size), device=input.device)
        ...
        for i in range(bz):
            ...
            for d in range(D):
                # Prepare frame (H, W, 3)
                frame = sam_input[i, :, d].permute(1, 2, 0).cpu().numpy()
                
                # Generate masks with SAM
                anns = self.mask_generator.generate(frame)
                
                # Filter masks with IoU > 0.9 and select top 11
                valid_masks = [ann for ann in anns if ann['predicted_iou'] > 0.90]
                valid_masks.sort(key=lambda x: x['predicted_iou'], reverse=True)
                selected_masks = valid_masks[:11]
                
                # Store masks
                for j, ann in enumerate(selected_masks):
                    mask = torch.from_numpy(ann['segmentation']).to(input.device)
                    sam_masks[i, j, d] = mask
        
        # Replace cam3D with SAM masks
        self.cam3D = sam_masks
        self.raw_cam = self.cam3D
        
        # Maintain original output structure
        bz, ch_n, D, H, W = self.cam3D.size()
        self.final_output = torch.ones(bz, ch_n, 1, 1, 1, device=input.device)
        self.gradcam = None
