...
img_size =(128,128)
video_len=29
def load_image_sequence(folder_path, start_num, end_num, downsample_factor=10):
  """
...
if __name__ == '__main__':
    ...
    output_folder_pkl = root_YTOBJ + "pkl2/"
    subsets = ['train', 'val']
    subsets = ['train']
...
# Inspect the contents
    ...
    # Access a specific variable (replace 'variable_name' with the actual variable name)
        ...
        for this_range in data_range.T:
            this_video_dir = root_YTOBJ + 'videos/'+category + "/"
            images, image_names = load_image_sequence(this_video_dir,this_range[0],this_range[1])
 
           
          
            # video_unique_color =get_unique_colors(np.array(masks)) 
            all_data =[]
            for img, msk in zip(images, image_names):
                # Organize as a dictionary or structure
               
                data_pair = {'frame': img, 'label': one_hot_vectors,'image_names': image_names}
