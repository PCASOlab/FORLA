...
class MOViDataset(Dataset):
    ...
    def _read_frames(self, idx, is_video=False):
        ...
        frames = []
        for n in range(num):
            frame_idx = start_idx + n * self.frame_offset
            file_path = filename.format(frame_idx)
            # file_path=osp.join(folder, file_path)
            # Check if the file exists
            if not osp.exists(file_path):
                raise FileNotFoundError(f"Frame {file_path} not found")

            # Load image and check if it can be opened
            try:
