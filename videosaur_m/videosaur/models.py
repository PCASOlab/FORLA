...
from working_dir_root import Video_len, Evaluation_slots,Fintune
Window_size=5
if Evaluation_slots:
    # from model.slots.temporal_slots_mask import Temporal_Binder
    from model.slots.temporal_slots import Temporal_Binder
...
def build(
    ...
    if Foundation_M == "DINO": 
        ...
        encoder = CLIPViT(model_name="ViT-B/16")
...
class ObjectCentricModel(pl.LightningModule):
    def __init__(
        ...
        *,
        ...
        self.temporal_binder = Temporal_Binder(slot_dim=self.initializer.dim, num_slots=7,num_frame=Window_size)
        # self.future_state_prdt = future_state_predit(slot_dim=64, num_slots=7,num_frame=5)
        self.future_state_prdt = Temporal_Binder(slot_dim=self.initializer.dim, num_slots=7,num_frame=Window_size)
    ...
    def forward(self, inputs: Dict[str, Any],features_stack=None, Using_bert =False,slot_ini = "rnn", Mask_feat = False,img_sim = False,epoch=0,read_id=None,Output_root=None,Student= False) -> Dict[str, Any]:
        encoder_input = inputs[self.input_key]  # batch [x n_frames] x n_channels x height x width
        assert encoder_input.ndim == self.expected_input_dims
        batch_size = len(encoder_input)
        # with torch.no_grad():
        ...
        if Using_bert:
            ...
            if (epoch%1)==0 or Evaluation_slots==True or Fintune ==True: # allow slots to be none bert reconstruction
                slots = temporal_slots_slide.apply_sliding_window_avg( self.temporal_binder,slots,window_size=Window_size, overlap=3)
