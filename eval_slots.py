...
IMG_Level_CORLOC = True
if Visdom_flag:
  viz = Visdom(port=8091)
from model.model_operator import post_process_softmask
from working_dir_root import Display_visdom_figure,selected_data
from  data_pre_curation. data_ytobj_box_train import apply_mask
from scipy.optimize import linear_sum_assignment
from scipy.ndimage import label
 
from scipy.spatial.distance import directed_hausdorff
from  videosaur_m.videosaur.metrics import ImageARI, VideoARI

# Example usage: ImageARI for image input
image_ari = ImageARI(
    video_input=False,
...
# Example usage: VideoARI for video input
video_ari = VideoARI(
...
def hungarian_iou_per_frame_instance(label_frame, predic_frame):
    ...
    """
    if torch.isnan(label_frame).any():
        return np.nan, np.nan, None, None, np.nan,np.nan 
    ...
    if not instance_masks.any():
        print("NO GT")
        return np.nan, np.nan, None, None, np.nan,np.nan  # No ground truth, return NaN for metrics
    ...
    matched_box_iou_percentage_IMG=0
    
    matched_box_iou_percentage_IMG =1.0 if matched_box_iou_count > 0 else 0


    return avg_dice, avg_hd, matched_gt_masks, matched_pred_masks, matched_box_iou_percentage,matched_box_iou_percentage_IMG
...
def process_metrics_from_excel(excel_path, output_root):
    ...
    metrics_df = metrics_df[['read_id', 'IoU', 'Frame_level_iou_instance', 'Frame-level average HD instance', 'Frame-level average ARI','Frame-level average corloc','Frame-level average corloc2']]
    ...
    for _, row in metrics_df.iterrows():
        ...
        current_group.append(row[['IoU', 'Frame_level_iou_instance', 'Frame-level average HD instance', 'Frame-level average ARI','Frame-level average corloc','Frame-level average corloc2']].values)
    ...
    group_averages_df = pd.DataFrame(group_averages, columns=['IoU', 'Frame_level_iou_instance', 'Frame-level average HD instance', 'Frame_level average ARI','Frame-level average corloc','Frame-level average corloc2'])
    ...
    overall_average = metrics_df[['IoU', 'Frame_level_iou_instance', 'Frame-level average HD instance', 'Frame-level average ARI','Frame-level average corloc','Frame-level average corloc2']].mean()
    ...
    metrics_list = ['IoU', 'Frame_level_iou_instance', 'Frame_level average HD instance', 
                   'Frame_level average ARI', 'Frame-level average corloc', 
                    'Frame-level average corloc2']

    # Process averages and standard deviations with scaling
    processed_overall_avg = []
    processed_std_between = []

    for metric, avg, std in zip(metrics_list, overall_average, std_between_groups):
        if metric == 'Frame_level average HD instance':
            # HD: keep original scale, round to 3 decimals
            scaled_avg = round(avg, 3)
            scaled_std = round(std, 3)
        else:
            # Other metrics: scale by 100 and round to 2 decimals
            scaled_avg = round(avg * 100, 2)
            scaled_std = round(std * 100, 2)
        processed_overall_avg.append(scaled_avg)
        processed_std_between.append(scaled_std)

    summary_df = pd.DataFrame({
        'Metric': metrics_list,
        'Overall Average': processed_overall_avg,
        'Standard Deviation Between Groups': processed_std_between
    })

    # Create mean ± std column with consistent formatting
    summary_df['Mean ± Std'] = summary_df.apply(
        lambda row: f"{row['Overall Average']:.2f} ± {row['Standard Deviation Between Groups']:.2f}" 
        if row['Metric'] != 'Frame_level average HD instance' 
        else f"{row['Overall Average']:.3f} ± {row['Standard Deviation Between Groups']:.3f}", 
        axis=1
    )
    ...
    # Print averages with consistent formatting
    print("\nOverall Averages:")
    for _, row in summary_df.iterrows():
        if row['Metric'] == 'Frame_level average HD instance':
            print(f"{row['Overall Average']:.3f}")
        else:
            print(f"{row['Overall Average']:.2f}")
...
def custom_stack_to_color_mask(mask_stack, H, W, pred_color_dict, matched_channels, 
                              alpha_matched=0.8, alpha_unmatched=0.7):
...
def cal_all_metrics_slots(read_id, Output_root, label_mask, predic_mask_3D,input_video_OG):
    ...
    pred_labels = predic_mask_3D.permute(1, 0, 2, 3).argmax(dim=1)  # [D, H, W]
    one_hot_pred = F.one_hot(pred_labels, num_classes=ch_ini).permute(0, 3, 1, 2).float()
    predic_mask_3D_onehot = one_hot_pred.permute(1, 0, 2, 3).contiguous()
    if Using_onehot == False:
        
        predic_mask_3D = predic_mask_3D > mask_threshold
        predic_mask_3D = torch.clamp(predic_mask_3D, 0, 1)
    else:
        predic_mask_3D = predic_mask_3D_onehot
    ...
    if "MOVIE" in selected_data or "MOVID" in selected_data:
        video_ari.update(
                true_mask=filtered_label_mask.permute(1, 0, 2, 3).unsqueeze(0),  # shape becomes [1, 14, 224, 224]
                pred_mask=predic_mask_3D_onehot.permute(1, 0, 2, 3).unsqueeze(0)  # likewise
            )
        Video_ARI = video_ari.value
        print("ARI",Video_ARI)
    ...
    for frame_idx in range(D):
        ...
        avg_iou_frame_instance,avg_HD_frame_instance,match_GT,matched_pred, corloc,corloc2= hungarian_iou_per_frame_instance(label_frame, predic_frame)
        avg_dice_frame_ari = get_ari_multichannel(label_frame, predic_frame)
        # image_ari.update(
        #     true_mask=label_frame .unsqueeze(0),  # shape becomes [1, 14, 224, 224]
        #     pred_mask=predic_frame. unsqueeze(0)  # likewise
        # )
        # avg_dice_frame_ari = image_ari.value
        ...
        frame_level_corloc_instance2.append(corloc2)
...
# Generate color mask using custom coloring
        ...
        if video_stack_matched_gt  :
            ...
            viz.image(np.transpose(combine_stack_matched.astype((np.uint8)), (2, 0, 1)), opts=dict(title=f'{read_id} - stack_color_mask'))
    ...
    if "MOVIE" in selected_data or "MOVID" in selected_data:
        avg_frame_level_ari = Video_ARI[0].cpu().numpy()

    avg_frame_level_corloc= np.nanmean(frame_level_corloc_instance)
    avg_frame_level_corloc2= np.nanmean(frame_level_corloc_instance2)
    ...
    print(f"Frame-level average corloc2: {avg_frame_level_corloc2:.4f}")
    ...
    metrics_video_data.append({
        ...
        'Frame-level average corloc2': avg_frame_level_corloc2,
