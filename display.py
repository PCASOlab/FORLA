...
import eval
import eval_box
from visual import Visdom_default_colors
import eval_slots
    ...
    def train_display(self,MODEL_infer,mydata_loader, read_id,Output_root):
        ...
        if (selected_data == "YTOBJ" and Evaluation):
            ...
            self.result_corr=eval_box.cal_all_metrics_box(read_id,Output_root,self.dataLoader.this_masks,frame_label,video_label, 
                                 self.Model_infer.cam3D[bz-1].detach(),self.Model_infer.output[bz-1,:,0,0,0].detach(),self.Model_infer.direct_frame_output,self.dataLoader.input_videos[bz-1],self.results)
        ...
        if (Evaluation_slots == True):
            ...
            self.result_corr=eval_slots.cal_all_metrics_slots(read_id,Output_root,label_mask,self.Model_infer.cam3D[bz-1],self.dataLoader.input_videos[bz-1])
        ...
        if   (Evaluation or Evaluation_slots):
            ...
            if gt_color_mask is not None:
                ...
                if Save_flag == True:
                    ...
                    viz.image(np.transpose(gt_color_mask.astype((np.uint8)), (2, 0, 1)), opts=dict(title=f'{read_id} - gt_color_mask'))
...
def stack_to_color_mask(final_mask, Ori_H, Ori_W, output_0, label_0, step_l):
    ...
    for j in range(len(final_mask)):
        ...
        if j >= len(Visdom_default_colors):
            ...
            color = list(Visdom_default_colors.values())[j]

        # Apply the color to the mask where this_mask_channel > 0.3
        color_mask[this_mask_channel > 0.3] = color

        mask_binary = (this_mask_channel[0] > 0.3)
        # Add white slot index number at center (for visibility)
        coords = np.argwhere(mask_binary)
        # add index to the mask
        if coords.size > 0:
            center_yx = coords.mean(axis=0).astype(int)
            cy, cx = center_yx
            if cy < H_m and cx < W_m:
                cv2.putText(color_mask[0], str(j), (cx, cy),
